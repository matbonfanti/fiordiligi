MODULE PotentialModule
   ...
   PRIVATE
   PUBLIC :: ComputePotential, SetupPotential, VHSticking, ThermalEquilibriumConditions
...
!> \name MASSES
!> Masses of the hydrogen and the carbon atoms, stored during setup and used to compute accelerations from forces
!> @{
   REAL :: MassCarbon              !< Carbon mass ( atomic units )
   REAL :: MassHydrogen            !< Hydrogen mass ( atomic units )
!>  @}
   ...
   CONTAINS
      ...
      SUBROUTINE SetupPotential( HMass, CMass )
         IMPLICIT NONE
         REAL, INTENT(IN) :: Hmass, Cmass            ! input masses of H and C in atomic units 
...
! ************************************************************************************
   ...
   END SUBROUTINE ThermalEquilibriumConditions
      ...
      SUBROUTINE ComputePotential( HX, HY, HZ, CZ, vv, axh, ayh, azh, azc )
         IMPLICIT NONE

         REAL, INTENT(IN) :: HX, HY, HZ              ! input coords of H in atomic units
         REAL, INTENT(IN), DIMENSION(121) :: CZ      ! input coords of the C atoms in atomic units
         REAL, INTENT(OUT) :: vv                     ! output potential in atomic units
         REAL, INTENT(OUT) :: axh, ayh, azh          ! output H acceleration in atomic units
         REAL, INTENT(OUT), DIMENSION(121) :: azc    ! output C accelerations in atomic units

         ! temporary variables to store positions in Ang units
         REAL :: xh, yh, zh    
         REAL, DIMENSION(121) :: z

         REAL, DIMENSION(121) :: ddz, dvvdz

         REAL :: a, b, c1, c2, d0

         REAL :: dbdzc, dc1dzc, dc2dzc, dd0dzc, df2dr, dkrdzc, dkrdzh, drdxh
         REAL :: drdyh, dswdr, dvdzc, dvdzh, dvidzc, dvidzh, dvqdr, dvqdzc
         REAL :: dvqdzh, dvtdrho, dvtds1, dvtds2, dvvdxh, dvvdyh, dvvdzh
         REAL :: dzgdzc, dzmdzc
         ...
         ! Error if module not have been setup yet
         CALL ERROR( .NOT. PotentialModuleIsSetup, "PotentialModule.ComputePotential : Module not Setup" )

         ! Transform input coordinate from AU to Angstrom
         xh = HX * MyConsts_Bohr2Ang
         yh = HY * MyConsts_Bohr2Ang
         zh = HZ * MyConsts_Bohr2Ang
         z(:) =  CZ(:) * MyConsts_Bohr2Ang
   ...
   ! **************************************************************************************************
         ...
         drdxh=xh/rho
         drdyh=yh/rho
         dvvdxh=dvtdrho*drdxh
         dvvdyh=dvtdrho*drdyh
         dvvdzh=dvtds1
         dvvdz(1)=dvtds2+ddz(1)-rkc*(z(1)-qqq)
         dvvdz(2)=-(dvtds1+dvtds2)/3.0+ddz(2)+rkc*(z(1)-qqq)/3.0
         dvvdz(3)=-(dvtds1+dvtds2)/3.0+ddz(3)+rkc*(z(1)-qqq)/3.0
         dvvdz(4)=-(dvtds1+dvtds2)/3.0+ddz(4)+rkc*(z(1)-qqq)/3.0
         DO nn = 5, 121
            dvvdz(nn) = ddz(nn)
         END DO
...
!          END DO 
         ...
         dvvdxh   = dvvdxh * MyConsts_Bohr2Ang / MyConsts_Hartree2eV
         dvvdyh   = dvvdyh * MyConsts_Bohr2Ang / MyConsts_Hartree2eV
         dvvdzh   = dvvdzh * MyConsts_Bohr2Ang / MyConsts_Hartree2eV
         dvvdz(:) = dvvdz(:) * MyConsts_Bohr2Ang / MyConsts_Hartree2eV

         ! Compute accelerations dividing by masses (all in atomic units)
         axh    = -dvvdxh   / MassHydrogen
         ayh    = -dvvdyh   / MassHydrogen
         azh    = -dvvdzh   / MassHydrogen
         azc(:) = -dvvdz(:) / MassCarbon
